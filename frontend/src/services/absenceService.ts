...
class AbsenceService {
  ...
  calculateDurationWithHolidays(
    ...
    if (halfDayTime && start.getTime() !== end.getTime()) {
      // Halbe Tage nur für Einzeltage
      return 0;
    }
    ...
    // Convert holidays to a set of date strings for easy lookup
    const holidayDateStrings = new Set(holidays.map(h => h.date));

    while (current <= end) {
      const dayOfWeek = current.getDay();
      const dateString = this.formatDateForAPI(current);
      
      const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
      const isPublicHoliday = holidayDateStrings.has(dateString);
    ...
    // Apply half days explicitly
    if (halfDayTime) {
      return 0.5;
    }

    // Apply dec 24 and dec 31 if they fell on a working day
    let finalCount = count;
    current = new Date(start);
    const dec24IsHalf = settings?.dec_24_is_half_day ?? true;
    const dec31IsHalf = settings?.dec_31_is_half_day ?? true;
    ...
    while (current <= end) {
      ...
      if (!isWeekend && !isPublicHoliday) {
        if (current.getMonth() === 11) { // 0-indexed, so 11 = December
          if (current.getDate() === 24 && dec24IsHalf) {
            finalCount -= 0.5;
          } else if (current.getDate() === 31 && dec31IsHalf) {
            finalCount -= 0.5;
          }
        }
      }

    ...
    for emp in employees:
        calendar_employees.append(
            EmployeeCalendarInfo(
                ...
                calendar_color=emp.calendar_color or "#3b82f6",
                active=emp.active,
...
def get_vacation_summary(
    ...
    # Get carryover days from VacationAllowance for this year
    from app.models.vacation_allowance import VacationAllowance
    allowance_stmt = (
    ...
    allowance = session.exec(allowance_stmt).first()
    carryover_days = float(allowance.carryover_days) if allowance else 0.0
    annual_allowance = total_allowance  # from entitlement

    # Combine total = annual + carryover
    combined_total = annual_allowance + carryover_days
    ...
    approved_absences = session.exec(approved_stmt).all()
    approved_days = float(sum(a.duration_days for a in approved_absences))
    ...
    pending_absences = session.exec(pending_stmt).all()
    pending_days = float(sum(a.duration_days for a in pending_absences))

    taken_days = approved_days + pending_days
    remaining_days = combined_total - taken_days
    available_days = combined_total - approved_days - pending_days

    return VacationSummaryRead(
        total_allowance=combined_total,

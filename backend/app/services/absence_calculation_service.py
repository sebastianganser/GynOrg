...
class AbsenceCalculationService:
    ...
    def calculate_absence_duration(
        self, 
        start_date: date, 
        end_date: date, 
        ...
        """
        Calculate the true vacation duration considering:
         - Weekends
         - Public Holidays (based on CalendarSettings employer_federal_state)
         - Dec 24 and Dec 31 half-day settings
         - Explicit half_day_time (AM/PM)
        """
        if start_date > end_date:
            return 0.0

        if half_day_time and start_date != end_date:
            raise ValueError("Halbe Tage sind nur bei eintägigen Abwesenheiten erlaubt.")

        from app.models.calendar_settings import CalendarSettings

        settings = self.session.exec(select(CalendarSettings)).first()
        fed_state = None
        dec_24 = True
        dec_31 = True
        ...
        if settings:
            try:
                fed_state = FederalState[settings.employer_federal_state] if settings.employer_federal_state else None
            except KeyError:
                pass
            dec_24 = settings.dec_24_is_half_day
            dec_31 = settings.dec_31_is_half_day

        working_calc = self.calculate_working_days_in_range(start_date, end_date, fed_state)
        
        duration = float(working_calc.working_days)

        if duration == 0:
            return 0.0

        # If it's a half-day request, the duration for the single day is 0.5
        if half_day_time:
            return 0.5
            
        # Deduct 0.5 for Dec 24 and Dec 31 if they fell on a working day
        current = start_date
        holiday_dates = {h.date for h in working_calc.holidays}
        while current <= end_date:
            if current.weekday() < 5 and current not in holiday_dates:
                if current.month == 12:
                    if current.day == 24 and dec_24:
                        duration -= 0.5
                    elif current.day == 31 and dec_31:
                        duration -= 0.5
    ...
    def get_vacation_balance(
        ...
        used_stmt = select(func.sum(Absence.duration_days)).select_from(
            ...
            and_(
                ...
                AbsenceType.is_vacation == True
            )
        )
        used_days = self.session.exec(used_stmt).first() or 0.0
        
        # Calculate pending vacation days
        pending_stmt = select(func.sum(Absence.duration_days)).select_from(
            Absence.__table__.join(AbsenceType.__table__)
        ).where(
            and_(
                Absence.employee_id == employee_id,
        ...
        pending_days = self.session.exec(pending_stmt).first() or 0.0

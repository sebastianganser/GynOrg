...
class HolidayService:
    ...
    def get_holidays_in_date_range(self, start_date: date, end_date: date, federal_state: Optional[FederalState] = None, include_school_vacations: bool = False) -> List[Holiday]:
        """Gibt alle Feiertage in einem Datumsbereich zurück.
        
        By default only PUBLIC_HOLIDAY entries are returned (for duration calculations).
        Set include_school_vacations=True to also get SCHOOL_VACATION entries.
        """
        query = select(Holiday).where(
        ...
        if not include_school_vacations:
            query = query.where(Holiday.holiday_type == HolidayType.PUBLIC_HOLIDAY)

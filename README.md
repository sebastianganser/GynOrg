...
**Login-Daten:** Siehe `.env`-Datei (Variablen `ADMIN_USERNAME` / `ADMIN_PASSWORD_HASH`)
...
# 1. Login mit den in .env konfigurierten Zugangsdaten
...
### **Authentifizierung**
- **Single User System**: Benutzername wird über `ADMIN_USERNAME` in `.env` konfiguriert
- **Passwort**: Bcrypt-Hash wird über `ADMIN_PASSWORD_HASH` in `.env` konfiguriert
- **JWT Tokens**: 30 Minuten Gültigkeit (konfigurierbar)
...
```bash
# Korrekte Anmeldedaten aus .env-Datei verwenden:
# Prüfe ADMIN_USERNAME und ADMIN_PASSWORD_HASH in .env
# Passwort-Hash generieren: python -c "import bcrypt; print(bcrypt.hashpw(b'DEIN_PASSWORT', bcrypt.gensalt()).decode())"
```
